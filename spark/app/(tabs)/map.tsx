...
export default function MapScreen() {
  ...
  // helper to snap coordinates to roads
  async function snapToRoads(path: LatLng[]): Promise<LatLng[]> {
  ...
  const accurateRoute = async (
    ...
    try {
      ...
      // Extract detailed coordinates from each step (denser data)
      ...
      // Snap all step points to roads for max accuracy
      ...
      const distance =
        Math.round(
          route.legs.reduce((s: number, l: any) => s + l.distance.value, 0) / 10
        ) / 100;
  ...
  const refreshEvents = async () => {
    ...
    Alert.alert("Refreshed", "Events updated");
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.controlsRight}>
        ...
        {/* NEW: Refresh events button */}
        <TouchableOpacity style={styles.fab} onPress={refreshEvents}>
          <Feather name="calendar" size={22} color="#444" />
        </TouchableOpacity>
